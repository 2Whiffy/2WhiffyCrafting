...
RegisterNetEvent('it-crafting:client:placeCraftingTable', function(tableItem, metadata)
    ...
    lib.showTextUI('[W/A/S/D] Move | [Mouse Scroll] Rotate | [E] Place | [G] Cancel', {
    ...
    while not placed do
        ...
        DisableControlAction(0, 30, true) -- A
        DisableControlAction(0, 31, true) -- S
        DisableControlAction(0, 32, true) -- W
        DisableControlAction(0, 33, true) -- D
        ...
        if IsControlPressed(0, 32) then -- W
            ...
            currentX = currentX + (math.sin(math.rad(heading)) * moveSpeed)
            currentY = currentY + (math.cos(math.rad(heading)) * moveSpeed)
        end
        ...
        if IsControlPressed(0, 34) then -- A
            local heading = GetEntityHeading(ped)
            currentX = currentX - (math.cos(math.rad(heading)) * moveSpeed)
            currentY = currentY + (math.sin(math.rad(heading)) * moveSpeed)
        end
