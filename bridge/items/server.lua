...
lib.callback.register('it-crafting:server:giveItem', function(source, item, amount, metadata)
...
lib.callback.register('it-crafting:server:removeItem', function(source, item, amount, metadata)
...
function it.createUsableItem(item, cb)
    ...
    -- For ox_inventory, usable items are handled through exports in item definitions
    -- The export is already registered in sv_usableitems.lua
    if it.inventory == 'ox' then
        ConsumableItems[item] = cb
        if Config.Debug then lib.print.info('[it-crafting] Registered ox_inventory usable item: ' .. item) end
        return
    end
