...
RegisterNetEvent("it-crafting:client:showCraftingMenu", function(type, data)
    ...
    if Config.LevelsSystem.enabled and recipe.levelRequirement then
        ...
        local playerLevel = exports['2WhiffyCrafting']:getPlayerLevel(recipe.levelRequirement.skillType)
        local hasRequirement = playerLevel >= recipe.levelRequirement.level

        table.insert(options, {
            title = "Level Requirement",
            description = ('%s Level %d %s'):format(
                skillLabel,
                recipe.levelRequirement.level,
                hasRequirement and '✅' or '❌'
            ),
            icon = hasRequirement and "check" or "times",
            disabled = true
        })
    end
