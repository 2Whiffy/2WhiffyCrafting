...
local function detectTargetSystem(target)
    ...
    if target == 'autodetect' then
        ...
        if not lib.table.contains(cache.supportedTargetSystems, target) then
            lib.print.error('['..it.name..'] The selected target system is not supported: ' .. target)
            return false
        else
...
if Config.Inventory == 'autodetect' then
    ...
    if not result then
        lib.print.error('['..it.name..'] Cannot find the resource for the selected inventory: ' .. inventory)
    else
        lib.print.info('['..it.name..'] Using configured inventory: ' .. it.inventory)
    end
end

if Config.Target == 'autodetect' then
    if not Config.Target then it.target = nil return end

    local target = detectTargetSystem('autodetect')
    if not target then
        lib.print.error('['..it.name..'] No supported target system detected! Did you rename your target system resource?')
    else
        if Config.Debug then lib.print.info('['..it.name..'] Detected target system: ' .. it.target) end
    end
else
    local target = Config.Target
    local result = detectTargetSystem(target)
    if not result then
        lib.print.info('['..it.name..'] Cannot find the resource for the selected target system: ', target)
    else
        if Config.Debug then lib.print.info('['..it.name..'] Detected target system: ', it.target) end
    end
end
